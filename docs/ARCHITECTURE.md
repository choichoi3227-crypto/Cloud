...
Console and admin pages on `bridge-console.{domain}` require an active session. Anonymous page requests are redirected to `sso.{domain}/login?next=...`, while the same console paths on `bridge.{domain}` or `sso.{domain}` remain 404 to preserve domain isolation.
