import { catalog, store, json, html, sha256, uid, body, sanitizeEmail, publicUser, priceOf, requireUser, audit, CloudPressKV, CloudPressSQL, CP3, ensureServicePlatform, serviceSnapshot, isAdmin, serviceNamespaces, currentUser, ensureAdminRole, adminEmailOf, ensureHydrated, registerRequestContext, scheduleSync, githubSyncStatus } from './platform.js';
...
export default { async fetch(req, env, ctx){
  registerRequestContext(env, ctx && ctx.waitUntil ? ctx.waitUntil.bind(ctx) : null);
  await ensureHydrated(env); // Worker(isolate)가 새로 떴다면 GitHub repo에서 마지막 데이터 스냅샷을 복원
  ...
  const page=resolvePage(url, scope); if(page){ if(scope==='console' && !(await currentUser(req, env))) return redirectToSsoLogin(url, env); return html(shell(page)); }
...
async function api(req,env,url,scope){
  if(url.pathname==='/api/health') return json({ok:true, service:'cloudpress', scope, targetAvailability:'99.99%', durableObjects:false, storage:'memory(main) + github-repo-json(backup), no cloudflare KV/D1/R2/DO', githubSync:githubSyncStatus(env), time:new Date().toISOString()});
  ...
  if(url.pathname==='/api/auth/lost-password' && req.method==='POST') return lostPassword(req,env);
  const gate=await requireUser(req, env); if(gate.error) return gate.error; const user=gate.user;
  if(url.pathname==='/api/me') return json({user:publicUser(user)});
  if(url.pathname==='/api/notices' && req.method==='POST') return createNotice(req,user,env);
  ...
  if(url.pathname==='/api/orders' && req.method==='POST') return createOrder(req,user,env);
  ...
  if(url.pathname==='/api/accounts' && req.method==='PATCH') return updateAccount(req,user,env);
  if(url.pathname==='/api/instances' && req.method==='GET') return json({instances:[...store.instances.values()].filter(i=>i.userId===user.id)});
  if(url.pathname==='/api/instances' && req.method==='POST') return createInstance(req,user,env);
...
function seedNotices(){if(!store.notices.size){const n={id:'notice_bootstrap',title:'CloudPress platform initialized',body:'도메인별 페이지 라우팅과 핵심 API가 구성되었습니다.',createdAt:new Date().toISOString()};store.notices.set(n.id,n);scheduleSync()} return [...store.notices.values()]}
async function signup(req,env){const b=await body(req); for(const f of ['email','name','address','password']) if(!b[f]) return json({error:`${f} is required`},400); const email=sanitizeEmail(b.email); if(store.users.has(email)) return json({error:'email already exists'},409); const user={id:uid('user'),email,name:String(b.name),address:String(b.address),passwordHash:await sha256(String(b.password)),roles:email===adminEmailOf(env)?['admin']:['user'],providers:['email'],createdAt:new Date().toISOString()}; store.users.set(email,user); audit(user.id,'user.signup'); scheduleSync(env); return json({user:publicUser(user)},201)}
async function login(req,env){const b=await body(req); const email=sanitizeEmail(b.email); const user=store.users.get(email); if(!user || user.passwordHash!==await sha256(String(b.password||''))) return json({error:'invalid credentials'},401); ensureAdminRole(user, env); const sid=uid('sess'); const ttl=Number(env.SESSION_TTL_SECONDS||86400); store.sessions.set(sid,{email,expiresAt:Date.now()+ttl*1000}); audit(user.id,'auth.login'); return json({user:publicUser(user)},200,{'set-cookie':`cp_session=${sid}; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=${ttl}${cookieDomain(env)}`})}
async function lostPassword(req,env){const b=await body(req); const email=sanitizeEmail(b.email); const user=store.users.get(email); if(user){const temp=crypto.randomUUID().replaceAll('-','').slice(0,16); user.passwordHash=await sha256(temp); audit(user.id,'auth.password_reset_requested',{provider:'firebase-email'}); scheduleSync(env); return json({ok:true, delivery:'firebase-email', developmentTempPassword:temp})} return json({ok:true})}
async function createOrder(req,user,env){const b=await body(req); const amount=priceOf(b); if(amount===null) return json({error:'unknown product'},400); const free=isAdmin(user); const order={id:uid('bill'),userId:user.id,product:b.product,plan:b.plan||null,gb:b.gb||null,amountUsd:free?0:amount,adminFree:free,status:free?'approved':'pending',cartPath:`/${b.product}/cart/`,createdAt:new Date().toISOString()}; order.cartPath+=order.id; store.orders.set(order.id,order); audit(user.id,'order.create',{orderId:order.id,product:b.product}); scheduleSync(env); return json({order,cartPath:order.cartPath},201)}
async function createInstance(req,user,env){const b=await body(req); const type=b.type||'wordpress'; const instance={id:uid('inst'),userId:user.id,type,name:b.name||`${type}-site`,status:'provisioning',network:{cdn:'cloudflare',dns:'automatic'},runtime:type==='wordpress'||type==='php'?'php-wasm':'static',database:type==='wordpress'?'CloudPressDB wordpress-nosql':null,storage:type==='wordpress'?'CP3 separate product':null,createdAt:new Date().toISOString()}; store.instances.set(instance.id,instance); audit(user.id,'instance.create',{instanceId:instance.id}); scheduleSync(env); return json({instance},201)}
...
async function createNotice(req,user,env){if(!isAdmin(user)) return json({error:'admin required'},403); const b=await body(req); const notice={id:uid('notice'),title:String(b.title||'공지'),body:String(b.body||''),createdAt:new Date().toISOString(),author:user.email}; store.notices.set(notice.id,notice); scheduleSync(env); return json({notice},201)}
...
async function updateAccount(req,user,env){const b=await body(req); if(b.name) user.name=String(b.name); if(b.address) user.address=String(b.address); audit(user.id,'account.update'); scheduleSync(env); return json({user:publicUser(user)})}
