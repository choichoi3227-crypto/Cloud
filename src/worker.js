...
const consoleRoutes = new Set(['/dashboard', '/instances', '/instance-detail', '/payments', '/billing', '/accounts', '/admin']);
...
async function api(req,env,url,scope){
  ...
  if(url.pathname==='/api/notices') return json({notices:seedNotices()});
...
function billingFor(user){const orders=[...store.orders.values()].filter(o=>o.userId===user.id); return {currency:'USD',totalUsd:orders.reduce((s,o)=>s+o.amountUsd,0),orders}}
